...
var isSaaS = window.location.hostname.indexOf("qlikcloud") > 0; 
var config = {
       ...
       initialProperties : {
					...
					if (isSaaS) {
						app.getAppLayout(function(layout){
							...
							var appLayout = {	
								...
								"qDocId": layout.qFileName,
								"qDocName": layout.qTitle + ".qvf",
					...
					if (appId != null) {
						...
						var appsFiltered = $scope.apps.filter(function (app) {
							...
							if ($scope.filterValue == undefined || $scope.filterValue.length == 0) {
								...
								var filterLower = $scope.filterValue.toLowerCase();
								var appNameLower = app.qDocName.toLowerCase();
								var re = new RegExp(filterLower.replace('*', '.*').replace('?', '.'), 'g');

								return appNameLower.match(re);
		...
		//		console.log(appsResolved) ;
			...
			function processApp ( appData, detailedJSON ) {
				...
				currApp.getAppLayout(function(layout) {
					...
					$q.all([objectPromise, meaListPromise, dimListPromise, masterObjectPromise,
						variablePromise, bookmarkPromise, scriptPromise, apppropsPromise]).then(function() {
			...
			function exportJSON ( apps ) {
				...
				var app = apps[0];
				var json = JSON.stringify(app, null,'\t');
